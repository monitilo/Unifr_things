...
class smAnalyzer(pg.Qt.QtGui.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.backgroundSizeLabel = QtGui.QLabel('Background Size:')
        self.backgroundSizeEdit = QtGui.QLineEdit()
        ...
        self.layout.addWidget(self.backgroundSizeLabel, 11, 0, 1, 1)
        self.layout.addWidget(self.backgroundSizeEdit, 11, 1, 1, 2)
        self.layout.addWidget(self.channelDifferenceLabel, 12, 0, 1, 1)
        self.layout.addWidget(self.channelDifferenceEdit, 12, 1, 1, 2)
        self.layout.addWidget(self.btn5, 14, 0, 1, 3)
    ...
    def importImage(self):
        ...
        f = filedialog.askopenfilename(filetypes=(("", "*.tiff"), ("", "*.tif")))
    ...
    def ROImean(self):
        ...
        cmap = pg.ColorMap(pos=np.linspace(0.0, 1.0, 6), color=colors)
        self.imv.setColorMap(cmap)
        self.w.setWindowTitle('SMAnalyzer - ROI Mean')
        self.imv.view.removeItem(self.roi)
    ...
    def detectMaxima(self):
        ...
        self.bgroiSize = np.array(self.roiSize)+[int(self.backgroundSizeEdit.text())*2]
        ...
        for i in np.arange(0, self.maxnumber):
            ...
            self.bgRoi[i,0] = pg.ROI((corrMaxima - [int(self.backgroundSizeEdit.text()), int(self.backgroundSizeEdit.text())]), self.bgroiSize, scaleSnap=True, translateSnap=True, movable=False)
    ...
    def exportTraces(self):
        ...
        for i in np.arange(0, self.maxnumber):
            for j in np.arange(1):
                ...
                bgNorm[i,j] = (int(self.moleculeSizeEdit.text())**2)*(bg[i,j])/(4*(int(self.backgroundSizeEdit.text()))*((int(self.backgroundSizeEdit.text()))+(int(self.moleculeSizeEdit.text()))))
