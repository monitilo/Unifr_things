# -*- coding: utf-8 -*-
"""
Created on Fri Aug 20 14:57:36 2019
...
con esa traza o set de datos que se encuentra graficada. Por default TODAS las
trazas arrancan en good trace, por lo tanto sólo hay que eliminar las que no se
desean.
...
#        print(self.selection)
        self.colorgraph = (120, 220, 50) #green
...
#        self.EndFrameEdit.setText(str(self.data.shape[0]))
    ...
    def showTrace(self):
        ...
        if self.selection[int(self.traceSlider.value()), 1] == 1:
            ...
            self.colorgraph = (250, 150, 50) #orange
        else:
            self.colorgraph = (100, 150, 255) #blue
    ...
    def PlotBinaryTrace(self):
        ...
        binary_trace_raw = np.zeros(self.data.shape[0])
        self.binary_trace = np.zeros(self.data.shape[0])
        ...
        binary_trace_raw = np.where(trace < threshold, binary_trace_raw, 1)

        localization_index = np.where(binary_trace_raw>0)[0] #indice de la traza donde hay localizaciones
        diff = localization_index [1:]-localization_index [0:-1]
        diff = diff-1
        #diffyb = [b[:-1], diff]
        diff1 = np.where(diff==1)
        agregaren = localization_index[diff1]+1
        localization_index_change = np.append(localization_index ,agregaren)
        localization_index_change = sorted(localization_index_change)
        self.binary_trace[localization_index_change] = 1
        
        self.BinaryTrace.plot(self.binary_trace, pen=pg.mkPen(color=(125 ,50, 150), width=1))
    ...
    def update_threshold(self):
        ...
        mode = stats.mode(self.data[:, int(self.traceSlider.value())])[0]
        trace = self.data[:, (int(self.traceSlider.value()))]
        new_binary_trace_raw = np.zeros(self.data.shape[0])   
        ...
        new_binary_trace_raw = np.where(trace < new_threshold, new_binary_trace_raw, 1)
        
        localization_index = np.where(new_binary_trace_raw>0)[0] #indice de la traza donde hay localizaciones
        diff = localization_index [1:]-localization_index [0:-1]
        diff = diff-1
        #diffyb = [b[:-1], diff]
        diff1 = np.where(diff==1)
        ...
        localization_index_change = np.append(localization_index ,agregaren)
        localization_index_change = sorted(localization_index_change)
        new_binary_trace[localization_index_change] = 1
    ...
    def Calculate_TON_times(self):
        ...
        for j in range(0, int(self.selection.shape[0])):
            ...
            if int(self.selection[j, 1]) == 1:
                ...
                print(int(self.selection[j,3])) 
                print(int(self.selection[j,4]))
                trace = trace[int(self.selection[j,3]):int(self.selection[j,4])]
                threshold = self.selection[j, 2]
                binary_trace_raw = np.zeros(int(trace.shape[0]), dtype = int)
                binary_trace = np.zeros(int(trace.shape[0]), dtype = int)
                ...
                localization_index_change = np.append(localization_index ,agregaren)
                localization_index_change = sorted(localization_index_change)
                binary_trace[localization_index_change] = 1
    ...
    def exportTraces(self):
        ...
        print('[Ton and Toff saved]: Amount of Ton saved: '+ str(self.times_frames_total_on.shape[0]))
...
#    app = pg.Qt.QtGui.QApplication([])
    ...
    if not pg.Qt.QtGui.QApplication.instance():
        app = pg.Qt.QtGui.QApplication([])
    else:
