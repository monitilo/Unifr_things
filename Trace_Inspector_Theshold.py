...
se carga un umbral determinado como la moda + std de cada traza que luego se
ira cambiando. 
"""
import numpy as np
...
#        self.meanEndEdit = QtGui.QLineEdit()
        ...
        layout.addWidget(threshold_index_Slider,           6.5, 0, 1, 3)
        layout.addWidget(self.threshold_index_Slider_Edit, 6.5, 1, 1, 3)
        layout.addWidget(self.thresholdSlider,             7, 0, 1, 3)       
        layout.addWidget(self.btnGoodTrace,                8, 0, 1, 1)
        layout.addWidget(self.btnBadTrace,                 8, 2, 1, 1)
        layout.addWidget(self.btnTonTimes,                 9, 0, 1, 3)
        layout.addWidget(self.btnExport,                   10, 0, 1, 3)
        layout.addWidget(self.graph,                       0, 4, 11, 100)
        layout.addWidget(self.BinaryTrace,                 12,0,500, 104)
...
#        layout.setColumnStretch(1,10)
    ...
    def importTrace(self):
        ...
        self.graph.clear()
        self.selection = np.zeros((self.data.shape[1], 4), dtype = int)
...
#        self.selection[:,3] = self.data.shape[0]
    ...
    # Pop up to show the binary trace of the selected trace
    def ShowBinaryTrace(self):
        self.BinaryTrace.show()
        self.BinaryTrace.setWindowTitle('Binary Trace')
