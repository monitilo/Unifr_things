# -*- coding: utf-8 -*-
"""
Created on Wed Nov  6 08:54:31 2019
...
def percolation(image, threshold): #Crea la matriz de clusters.
	R = image  # R_matrix(L,p);
	L = len(image)
	iD = 1;
	label = np.zeros((L,L));
	for i in range(0,L):
		...
		for j in range(0,L):
			if R[i,j]:
				...
				if left <= threshold and above <= threshold:
					label[i,j] = iD;
					iD = iD + 1;
				elif left > threshold and above <= threshold:
					label[i,j] = label[i,j-1];
				elif left <= threshold and above > threshold:
					label[i,j] = label[i-1,j];
				else:
